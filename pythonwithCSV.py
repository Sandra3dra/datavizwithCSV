...
with open("data/googeplaystore.csv") as csvfile:
	...
	for row in reader:
		...
		if line_count is 0: # this will be text, not data
			...
			line_count += 1 # increment the line count for the text loop
		else:
			#	grab all the ratings and push them into the array
			ratingsData = row[2]
			ratingsData = ratingsData.replace("NaN", "0")
			ratings.append(float(ratingsData)) #	float wil turn a string (place of text) into a number
